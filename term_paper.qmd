...
```{r downsampling_correction, include=FALSE}
...
```{r downsample_correction, include=FALSE}
...
```{r etre_difference, include=FALSE}
...
As the analysis required detailed (and very time consuming) manual annotation, we decided to split the annotation work between us. However, to ensure consistent annotation, a representative sample of one third of all instances (320; 33,25%) were double annotated, i.e. by both of us. For these instances, we calculated Cohen's ϰ (Kappa) as a measure of inter-rater agreement, (following @levshina_how_2015 [201] and the documentation in the psy package documentation (<https://cran.r-project.org/web/packages/psy/psy.pdf>).)

Data preprocessing

Combine the data

```{r data_combine, include=FALSE}
etre_sample_532_IAA <- read.csv(file = here("data", "etre_sample_532_IAA.csv"))
#se_faire_sample <- read.csv(file = here("data", "se_faire_sample_IAA.csv"))
#passive_sample_combined <- bind_rows(etre_sample_532, sefaire_sample) 
```

add "NA" as value for all cases that are only annotated by one person

```{r add_NA, include=FALSE}
etre_sample_532_IAA[etre_sample_532_IAA == ""] <- NA
...
library(psy)
kappa_animacy <- ckappa(etre_sample_532_IAA[c("subj_animacy_A", "subj_animacy_B")])
kappa_animacy #0.8555957

kappa_gender <- ckappa(etre_sample_532_IAA[c("subj_gender_A", "subj_gender_B")])
kappa_gender #0.8941021

kappa_agent_pp <- ckappa(etre_sample_532_IAA[c("agent_pp_A", "agent_pp_B")])
kappa_agent_pp #0.9118457

kappa_modal <- ckappa(etre_sample_532_IAA[c("modal_A", "modal_B")])
kappa_modal #0.96

kappa_tam <- ckappa(etre_sample_532_IAA[c("tam_A", "tam_B")])
kappa_tam #0.8323071

kappa_cluster <- ckappa(etre_sample_532_IAA[c("cluster_A", "cluster_B")])
kappa_cluster #0.7003745

kappa_adversativity <- ckappa(etre_sample_532_IAA[c("adversativity_A", "adversativity_B")])
kappa_adversativity #0.6122779
...
mean_kappa #0.8120668, very high agreement :)

```

The inter-rater agreement is high at 0.81.
