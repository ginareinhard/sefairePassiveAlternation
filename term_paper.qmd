...
The sample comprised 1064 observations: 532 for [se faire]{.smallcaps} and [être]{.smallcaps} each.

reasoning: why exclude unknown and mixed?
...
The AIC is 732.5, with an R^2^ of 0.46 when excluding random effects and 0.67 when accounting for them. Since the variance explained by the random effects is larger than zero, they must be kept.

$$
\begin{align*}
passiveVariant & \sim subjAnimacy + subjGender + agentPP + modal + tam + cluster \: + \\ & \qquad adversativity + subjAnimacy \cdot gender + subjAnimacy \cdot cluster \: + \\ & \qquad subjAnimacy \cdot adversativity + cluster \cdot adversativity \: + \\ 
...
#                                 GVIF Df GVIF^(1/(2*Df))
# subj_animacy                4.209237  1        2.051642
# subj_gender                 1.373802  1        1.172093
# agent_pp                    1.041076  2        1.010115
# modal                       1.044101  1        1.021813
# tam                         1.022810  1        1.011341
# cluster                    57.865906  2        2.758073
# adversativity               1.462643  1        1.209398
# subj_animacy:subj_gender    3.041097  1        1.743874
# subj_animacy:cluster        1.871213  2        1.169582
# subj_animacy:adversativity  2.578481  1        1.605765
# cluster:adversativity      50.563250  2        2.666605
```
...
In the first step, all non-significant interactions (`cluster:adversativity`, `subjAnimacy:cluster`, `subjAnimacy:subjGender`) were removed, improving the AIC improved to 725.4 with an R^2^ of 0.450 / 0.664. Thus, without the interactions, model accuracy increased while the explained variance decreased slightly. With $p$-values \> 0.05, the categories `subj_gender`, `agent_pp`, `modal` and `tam` were found to be non-significant.
...
```{r final-model-unknown, echo = FALSE}
...
summary(final_model_unknown) # AIC: 730.7
tab_model(final_model_unknown) # Marginal R2 / Conditional R2	0.448 / 0.658
