...
Both se faire and être passives are available for the active diathesis [@novakova_construction_2009 [108]] and assign the role of the undergoer to the subject referent. However, as @novakova_construction_2009 [108] notes, the constructions differ in semantic nuances: "Elles ont un sens proche mais pas identique." Both variants may be used in the same context/ sentence:
...
This demonstrates that the two passive variants are not semantically identical but rather potentially carry different meanings that may be more or less pronounced depending on the context: Previous research suggests that se faire passive is more likely to have a causative undertone while the être passive is more neutral. it has been proposed that constructions vary in their passivity ( "passive gradient", @bohmann_large-scale_2023 [34])) and that there is a continuum between causatity and passivity. However, this is what makes the analysis interesting and our research worthwhile - what are the exact factors that influence the choice between the the two variants?

This introduces certain difficulties: The passive/ causative continuum discussed in @sec-debate makes the definition of the variable context particularly challenging as some constructions may be more passive than others - Deciding where to draw the line between a passive and causative construction is a nuanced task.

@bohmann_large-scale_2023 [35] face similar challenges when defining the variable context of the be and get passive: "passive gradient" exist where both GET and BE may be used, but where the choice between the two entails a semantic difference", "definition of the variable context is not just a problem of automation, but of linguistic interpretation" \[36\].
...
The *complément d'agent* is optional. It is usually formed with *par*, sometimes also with *de*, and even more rarely with *dans*. We do not consider the special case with *dans* in our analysis. Semantically, the *complément d'agent* does not always refer to an agent. Verbs where the subject is an infinitive or a subordinate clause as well as certain state verbs occur with *de* in the passive voice. With a nominal subject, the passive with *de* is a remnant of Classical French and is therefore very rare, affecting only 2% of cases. In most cases, *de* alternates with *par* in contemporary French, although some verbs, such as *constituer*, naturally prefer *de*. For verbs like *aider*, a human *complément d’agent* is more natural than an abstract one. The verbs that allow this construction are often state verbs but belong to different semantic classes **(examples to be provided)**. The choice between *par* and *de* is not always interchangeable in meaning. For example, with the verb *accompagner*, a *complément d’agent* with *par* implies a more active role of the argument than with *de*. *de* is generally associated with non-dynamic, static, or non-agentive interpretations[@abeille2021grande 216-218].

We are aware that introducing the *complément d'agent* as a predictor in our analysis introduces an additional source of variance, along with potential interactions and potential collinearity. However, we have chosen to include it, as we believe it is relevant to subject responsibility.

We expect the presence or absence of the *complément d'agent* to influence passive choice. The *complément d'agent* introduces an external argument that is likely the driving force behind the action described: @kupferman_construction_1995 postulates an "agentivity requirement" for agents denoted by the *complément d'agent* with *par*, attributing a strong responsibility to the component. @hundt_be-_2024 and @bohmann_large-scale_2023 found that in English passive constructions, the Agent PP (**Abkürzung auflösen**) reduces subject responsibility by introducing another entity that is agentive and responsible.
...
passive_sample_without_unknown <- passive_sample_clean|> 
  filter(subj_gender != "mixed") |> 
...
subj_table <- table(passive_sample_without_unknown$subj_gender, passive_sample_without_unknown$passive_variant)
...
freq_gender <- passive_sample_without_unknown |> 
...
ggplot(passive_sample_without_unknown, 
...
freq_cluster <- passive_sample_without_unknown |> 
...
ftable(passive_sample_without_unknown$adversativity, passive_sample_without_unknown$passive_variant, passive_sample_without_unknown$subj_animacy)
```
...
passive_sample_without_unknown$subj_gender <- relevel(passive_sample_without_unknown$subj_gender, ref = "masculine")
passive_sample_without_unknown$subj_animacy <- relevel(passive_sample_without_unknown$subj_animacy, ref = "inanimate")
passive_sample_without_unknown$cluster <- relevel(passive_sample_without_unknown$cluster, ref = "neutral")
passive_sample_without_unknown$adversativity <- relevel(passive_sample_without_unknown$adversativity, ref = "non-adversative")

levels(passive_sample_clean$subj_gender)
levels(passive_sample_clean$agent_pp)
levels(passive_sample_without_unknown$subj_gender)
```
...
```{r model-without-unknown, output=FALSE}
passive_glmer_without_unknown <- glmer(
    passive_variant ~ (subj_animacy + subj_gender + agent_pp + modal + tam + cluster + adversativity + subj_animacy:subj_gender + subj_animacy:cluster + subj_animacy:adversativity + cluster:adversativity) +
               (1 | lemma) + (1 | speaker),
    family = "binomial", 
    data = passive_sample_without_unknown,
    control = glmerControl(optimizer = "bobyqa")
  )

summary(passive_glmer_without_unknown) #AIC: 731.2
tab_model(passive_glmer_without_unknown) # Marginal R2 / Conditional R2	0.462 / 0.668 // 0.704 / NA
vif(passive_glmer_without_unknown)
```
...
removing subj_animacy: subj_gender

```{r, output = FALSE}
model_1_without_unknown <- glmer(
    passive_variant ~ (subj_animacy + subj_gender + agent_pp + modal + tam + cluster + adversativity + subj_animacy:adversativity) + (1 | lemma),
    family = "binomial",
    data = passive_sample_without_unknown,
    control = glmerControl(optimizer = "bobyqa")
  )

summary(model_1_without_unknown) # AIC: 738
tab_model(model_1_without_unknown) #Marginal R2 / Conditional R2	0.453 / 0.659
```

removing non-significant fixed effects (agent_pp, modal, tam)

```{r model-2, output=FALSE}
model_2 <- glmer(
...
without subject gender (non-significant without unknown)

```{r}
final_model_without_unknown <- glmer(
    passive_variant ~ (subj_animacy + cluster + adversativity + subj_animacy:adversativity) + (1 | lemma),
    family = "binomial", 
    data = passive_sample_without_unknown,
    control = glmerControl(optimizer = "bobyqa")
  )

summary(final_model_without_unknown) # AIC: 730.7
tab_model(final_model_without_unknown) # Marginal R2 / Conditional R2	0.448 / 0.658
vif(final_model_without_unknown)
```

Intercept: 2.073421; meaning: chances of se faire are 2.073421 times greater than être given this context
...
#model_2
Hmisc::somers2(
  predict(final_model_without_unknown), as.numeric(passive_sample_without_unknown$passive_variant)-1
)["C"] 
# C 0.9388979 - pretty good I think? "outstanding discrimination" according to Hosmer & Lemeshow (2000: 162) in Levshina (2015: 259)
```
...
anova(model_2, null_model, test="Chisq")
#highly significant!!
#relative likelihood
(exp((AIC(null_model)-AIC(model_2))/2))
...
freq_communication <- passive_sample_without_unknown |> 
...
ggpredict(final_model_without_unknown, c("subj_animacy", "adversativity")) |> 
  plot()
...
```{r}
ranef <- as.data.frame(ranef(final_model_without_unknown))
