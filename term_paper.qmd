...
The [se faire]{.smallcaps} construction tends to occur with dynamic processes, focusing on cases where the source role is agentive and the target role is affected, which is the typical case of semantic transitivity [@raineri_analyse_2010 90, 238]. Multiple hypotheses are found in the literature concerning the compatibility of certain verbs with the [se faire]{.smallcaps} passive variant. For example, @kupferman_construction_1995 \[67\] and @raineri_analyse_2010 [90] state that the [se faire]{.smallcaps} passive works well with dynamic verbs but not with stative verbs or verbs of state change with an external "cause" argument, although exceptions are provided by @novakova_construction_2009 **pages?**, @martin_construction_2002 **pages?** and @raineri_analyse_2010 [100-103].
...
She also argues that [se faire]{.smallcaps} constructions with psychological verbs, such as in *Elle s'est fait séduire par Pierre* 'She was seduced by Pierre', are not always causative. @novakova_construction_2009 also notes that while rare, [se faire]{.smallcaps} constructions with stative verbs can sometimes be interpreted passively, as in *Il peine encore à se faire aimer par tous les siens* 'He still struggles to be loved by all his own'.

The semantic class of the verb has been shown to play a crucial role in the use of the passive voice in oral French. In the corpus study by @da2022alternance, 85% of *frapper* ('hitting-touching') class verbs occurred in passive constructions, while only 15% were used in the active voice. This is consistent with findings from journalistic English, where verbs describing violent actions are often used in the passive voice (**source**). According to @da2022alternance, this preference can be explained by two factors: (1) the topical character of the affected undergoer, which makes it more likely to be promoted to the subject position, and (2) the possibility to omit an unknown or less salient agent, which aligns with the concept of subject responsibility. We expect this effect to hold for the [se faire]{.smallcaps} passive as well. Even though both the [se faire]{.smallcaps} and [être]{.smallcaps} passives topicalise the undergoer, the [se faire]{.smallcaps} passive typically implies some degree of subject involvement, so highly dynamic verbs such as *frapper* might increase the likelihood that it is favoured over the [être]{.smallcaps} passive, in which the subject is a more passive patient.

Following @da2022alternance (+**2020**?), We decided to include the semantic class of the verb lemma as a factor, using the classification in the online dictionary "Les Verbes Français - LVF" **(Dubois&Dubois-Charlier 1997)**. As shown in @fig-clusters in the study by @da2022alternance the preference for active constructions was not statistically significant for location verbs, while the probability of communication verbs occurring in the active was significantly higher. Some classes showed no clear effect on passive frequency, with approximately equal distributions of active and passive forms (e.g. verbs of perception, cognition, psychological states, behaviour, movement and transfer). Locative and communicative verbs, on the other hand, appeared more frequently in the active voice.
