...
For example, in the following sentences from our sample from the ParlaMint 2.1 corpus [@11356/1431], in (1) the agent *nos concitoyens* appears in the *complément d'agent* introduced by *par*, while in (2) the agent is omitted:

\(1\) *\[...\]* *ils sont élus par nos concitoyens au suffrage universel.\
* '\[...\] they ar*'*e elected by our fellow citizens through universal suffrage.'

\(2\) *Les députés sont élus pour contrôler votre action.\
...
The *complément d'agent* introduces an external agent into the passive construction: it refers to the causer or instigator of the action and is derived through diathesis from the subject of the corresponding active sentence (see @sec-etre).

The *complément d'agent* is optional. It is usually formed with *par*, sometimes also with *de*, and even more rarely with *dans*. We do not consider the special case with *dans* in our analysis. Semantically, the *complément d'agent* does not always refer to an agent. Verbs where the subject is an infinitive or a subordinate clause as well as certain state verbs occur with *de* in the passive voice. With a nominal subject, the passive with *de* is a remnant of Classical French and is therefore very rare, affecting only 2% of cases. In most cases, *de* alternates with *par* in contemporary French, although some verbs, such as *constituer*, naturally prefer *de*. For verbs like *aider*, a human *complément d’agent* is more natural than an abstract one. The verbs that allow this construction are often state verbs but belong to different semantic classes **(examples to be provided in** @tbl-X**?****)**. The choice between *par* and *de* is not always interchangeable in meaning. For example, with the verb *accompagner*, a *complément d’agent* with *par* implies a more active role of the argument than with *de*. *de* is generally associated with non-dynamic, static, or non-agentive interpretations[@abeille2021grande 216-218].
...
| d'assistance:                                                                                 |           |
|                                                                                               |           |
| *aider*, *appuyer*, *assister*, *seconder*                                                    |           |
+-----------------------------------------------------------------------------------------------+-----------+
| donnant la composition d'un groupe:                                                           |           |
|                                                                                               |           |
| *composer*, *constituer*, *former*                                                            |           |
+-----------------------------------------------------------------------------------------------+-----------+
| instrumentaux:                                                                                |           |
|                                                                                               |           |
| *entourer*, *recouvrir*                                                                       |           |
...
| *précéder*, *suivre*                                                                          |           |
+-----------------------------------------------------------------------------------------------+-----------+
| d'opinion ou d'activité intellectuelle:                                                       |           |
|                                                                                               |           |
| *admettre*, *approuver*, *comprendre*, *connaitre*, *rejeter*, *tolérer*                      |           |
+-----------------------------------------------------------------------------------------------+-----------+
| de perception:                                                                                |           |
|                                                                                               |           |
| *écouter*, *entendre*, *voir*                                                                 |           |
+-----------------------------------------------------------------------------------------------+-----------+
| de sentiment:                                                                                 |           |
|                                                                                               |           |
| *aimer*, *apprécier*, *craindre*, *détester*, *envier*, *inquiéter*, *mépriser*, *surprendre* |           |
+-----------------------------------------------------------------------------------------------+-----------+

: main verbs with de passive (GGF 217)

We are aware that introducing the *complément d'agent* as a predictor in our analysis introduces an additional source of variance. However, we have chosen to include it, as we believe it plays a crucial role in subject responsibility.

We expect the presence or absence of the *complément d'agent* to influence the choice of the passive variant. The *complément d'agent* introduces an external argument that is likely the driving force behind the action described: @kupferman_construction_1995 postulates an "agentivity requirement" for agents denoted by the *complément d'agent* with *par*, attributing a strong responsibility to the component. @hundt_be-_2024 and @bohmann_large-scale_2023 found that in English passive constructions, the Agent PP (**Abkürzung auflösen**) reduces subject responsibility by introducing another entity that is agentive and responsible.
...
The intercept of the final model is 2.97. This means that if the subject is animate, the cluster of the verb is neutral and the construction has an adversative sense, the chances of se faire are 2.97 times greater than être.
...
subj_table <- table(passive_sample_clean$subj_gender, passive_sample_clean$passive_variant)
