...
-   extract se faire passive constructions with CQL expression, exclude erroneously tagged constructions that are not real se faire + Inf expressions (e.g.)
...
In general, we opted for a wide approach, including as many constructions as possible. This means that even when the causative reading seems natural (because the subject triggers the event), but there is a passive understanding (since the subject is also undergoing an action), we included the construction. In doing so, we deviate from the first criterion established by @bellec_construction_2014. Examples of such constructions include *se faire vacciner* "to (go) be/get vaccinated" and *se faire élire* "to be/get elected" or "to stand for election".
