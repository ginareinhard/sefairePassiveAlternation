...
Both se faire + Vinf and être + Vé are available for the active diathesis \[@novakova_construction_2009 \[108\]. However, as @novakova_construction_2009 [108] notes, the constructions differ in semantic nuances: "Elles ont un sens proche mais pas identique."

Following @bohmann_large-scale_2023 [35] who claim that "only cases where both variants can be used interchangeably should be considered", we aim to exclude constructions that do not allow for a paraphrase with the other.
...
The above discusses passive/ causative continuum makes the definition of the variable context particularly challenging as some constructions may be more passive than others - where to draw the line?
...
We follow @raineri_analyse_2010 [165] in including only se faire + Inf constructions that have a natural paraphrase with the être passive in order to include only constructions that directly contrast with être.
...
-   causative and passive: likely a continuum -\> therefore, many cases which are not clearly passive or causative or the subject is both the undergoer and the causer (se faire vacciner).

-   as @bohmann_large-scale_2023, we have faced difficulties in defining the variable context and had to rely on a fuzzy context. In general, we have chosen a wide approach and have opted to include as many constructions as possible: That means that even if the causative reading appears more natural but there is a passive understanding because the subject undergoes an action it is included (goes against first criterion by @bellec_construction_2014). Also verbs could be causative in one context and passive in the other?? But for practical reasons, we had to... For traceability, some high frequency unclear cases and our argumentation/ reasoning for our decision to include/ exclude (examples form corpus)

-   exclude:

    -   fixed highly idiomatic expressions such as "se faire passer pour" *to impersonate*, "se faire avoir" *to be fooled*, "se faire mousser" *to boast/ to brag"
    -   expressions where an être passive is not natural or has a different meaning (attendre: "L'indemnisation se fait attendre" *The compensation is taking a long time to arrive/ is long overdue/ is a long time coming* vs. "être attendu" *to be expected;)*
        -   se faire sentir: *make itself felt* / *be felt*, être senti *to be felt* -> mostly with "pour" -> no par-phrase possible (in general)
        - se faire connaitre *to make oneself known* vs. "être connu" *to be known*
           - se faire sentir pour

    difficult:

    -   se faire entendre *to make oneself heard/ be heard*, être entendre *to be heard*: context dependent, mostly clearly more causautive -> exclude?
    -   context dependent???
    - se faire taper
 
    - se faire sentir/ être senti -> 
    

-   include: se faire vacciner, se faire élire: *to (go) get vaccinated, to get elected* included because although the subject is agentive and induces/ triggers the action, they also undergo it.
