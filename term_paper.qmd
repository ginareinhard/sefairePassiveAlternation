...
> 5.  
>
>     a.  *Le coiffeur coupe les cheveux de Pierres.*\
...
However, whether constructions like 5b. count as true passives is controversial. Some scholars argue that the se faire construction is fundamentally causative rather than passive, since the subject both initiates and undergoes the action (as in: "Pierre made the hairdresser cut his hair"). More on that in @sec-debate.

There is a semantic tension between the fundamentally causative nature of the verb *faire* and the passive reading, which has been subject to several discussions in the literature. Labelle [-@labelle_anticausativizing_2013: 235] classifies the construction as “formally causative, but semantically passive”. The apparent contradiction has led scholars to question the nature of the construction, as Koenig and Pederson [-@koenig_semantic_1992: 145] put it: "how can a passive-like construction arise from a combination of a causative and a reflexive marker, where no marking of the passive function is apparent?".
...
## *être* and [se faire]{.smallcaps}: Competition or Complementation?
...
Previous research also suggests that se faire passive is more likely to have a causative undertone while the être passive is more neutral. As discussed in @sec-debate, it has been proposed that the constructions vary in their degree of passivity ( "passive gradient", @bohmann_large-scale_2023: 34\]) and that even within the [se faire]{.smallcaps} passive, there exists a continuum between causatity and passivity [@raineri_analyse_2010: 154-166].

This demonstrates that the two passive variants are not semantically identical but rather potentially carry different meanings. These meanings may be more or less pronounced depending on the context. This potential semantic difference introduces certain difficulties in the contrastive analysis of the alternation.
...
The AIC is 732.5, with an R^2^ of 0.46 when excluding random effects and 0.67 when accounting for them.
...
For our model, the concordance index is high at 0.94. Comparing this to the scale reported in Levshina [-@levshina_how_2015: 259], our model shows "outstanding discrimination".
...
```{r, echo=FALSE}
table(passive_sample_clean$adversativity, passive_sample_clean$passive_variant)

ftable(passive_sample_clean$adversativity, passive_sample_clean$passive_variant, passive_sample_clean$subj_animacy)
```

```{r animacy-adversativity-plots, echo = FALSE}
...
```{r data-exploration-plots, ECHO=FALSE}
...
If the subject is inanimate and non-adversative, the être passive is almost exclusively used and therefore far more likely than the se faire passive.

However, the likelihood of the se faire passive increases significantly when the construction has an adversative sense: In our study, se faire and être were equally likely for inanimate subjects and adversative contexts,.

In contast, constructions with animate subjects show a strong preference for the se faire passive regardless of adversativity: The chance of se faire being used is high for both inadversative and adversative contexts (and increases a litte but more for adversative contexts, but not significantly).
