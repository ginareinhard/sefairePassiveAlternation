...
The [se faire]{.smallcaps} passive construction, hereinafter called [se faire]{.smallcaps}, provides an alternative way to express passive meaning in French. When constructing a passive sentence, French speakers may choose to use [se faire]{.smallcaps} instead of the canonical [être]{.smallcaps}. [se faire]{.smallcaps} has been investigated and discussed by various scholars, such as @bellec_construction_2014, @gaatone_desagreable_1983, @koenig_semantic_1992, @kupferman_construction_1995, @labelle_anticausativizing_2013, @raineri_analyse_2010, @novakova_construction_2009, and @tasmowski-de_ryck_causatif_1987, to name a few.

[se faire]{.smallcaps} consists of the pronoun *se* (which shows agreement with the subject), a form of the verb *faire* ('to do'), and a verb in the infinitive.

> 3.  *Des policiers se font agresser.* (ParlaMint corpus)\
>     'Police officers were/ have been attacked.'

Similar to [être]{.smallcaps}, the direct object of the active sentence becomes the subject of the passive sentence. This subject is the affected experiencer of the action described (compare to example above). Different than with [être]{.smallcaps}, however, the diathesis is not restricted to the direct object: [se faire]{.smallcaps} has "greater syntactic flexibility" [@raineri_analyse_2010: 93-95] and can also occur with ditransitive verbs, where the indirect object becomes the subject:
...
[se faire]{.smallcaps} also allows possessor constructions, where the subject of the passive corresponds to part of the object in the active sentence:
...
However, constructions like 5b. are controversial: Some scholars (e.g. Sveberg [-@sveberg_se_2012] argue that such instances of [se faire]{.smallcaps} are fundamentally causative rather than passive, since the subject both initiates and undergoes the action (as in: "Pierre made the hairdresser cut his hair"). This debate will be outlined in @sec-debate.

The semantic tension between the fundamentally causative nature of the verb *faire* and the passive reading has been discussed extensively in the literature. Labelle [-@labelle_anticausativizing_2013: 235] classifies the construction as “formally causative, but semantically passive”. The apparent contradiction has led scholars to question the nature of the construction, as Koenig and Pederson [-@koenig_semantic_1992: 145] put it: "how can a passive-like construction arise from a combination of a causative and a reflexive marker, where no marking of the passive function is apparent?".
...
factors_passive <- c("subject animacy", "subject gender", "agent PP/ complément d'agent", "modal", "TAM", "cluster", "adversativity", "mean")
...
The levels are ordered alphabetically by default. We rearranged the reference levels to be most useful for both modelling and interpreting: Where possible, we chose the most standard case as a reference level (`passive_variant`: être; `agent_pp`: 0; `modal`: no; `tam`: non-progressive, cluster: `neutral`, subject animacy: `inanimate`). If this would negatively affect the stability of the model, the most frequent (`subject animacy`: animate,`subj_gender`: masculine) or the most evenly distributed case (`adversativity`: adversative) was selected.
...
Surprisingly, and contrary to our initial hypothesis, the only significant main effect in our model the semantic verb cluster, with communication verbs favouring [se faire]{.smallcaps} over [être]{.smallcaps}. We had predicted that hitting-touching verbs would favour [se faire]{.smallcaps}, given their dynamic and adversative nature, while communication verbs would be more compatible with [être]{.smallcaps}, as they typically feature an animate subject and frequently occur with object complements.

Specifically, the odds of [se faire]{.smallcaps} occurring with communication verbs are 5.98 times higher than with verb in the "neutral" category (CI: 1.95–18.33, p = 0.002). In contrast, hitting-touching verbs, which we expected to strongly favour [se faire]{.smallcaps}, did not show a significant effect. This means that no clear difference between [se faire]{.smallcaps} and [être]{.smallcaps} could be found for these verbs. @fig-cluster-model-plot shows the predicted probabilities of [se faire]{.smallcaps} for each verb cluster, along with the confidence intervals. Communication verbs have the highest predicted probability of [se faire,]{.smallcaps} **close to 95%**. This supports our surprising finding that [se faire]{.smallcaps} is preferred for these verbs, contrary to our hypothesis. Neutral verbs show a lower probability, **around 70%,** suggesting no strong preference for [se faire]{.smallcaps} over [être]{.smallcaps}. Hitting-touching verbs also have a probability **around 75%**, but their confidence interval overlaps significantly with that of neutral verbs, indicating no significant difference between these two categories. The wide confidence intervals for all verb cluster categories, particularly for hitting-touching verbs, suggest variability or uncertainty in the estimates, which aligns with the non-significant effect found for this category in our statistical model.
...
```{r cluster-model-plot, fig.cap = "Figure X: ...", echo = FALSE}
...
Looking at the most frequent communication verb lemmas in our data in @tbl-cluster-lemmas, we find that *insulter* (10 occurrences), *engueuler* (7 occurrences), and *représenter* (5 occurrences) appear most often with [se faire]{.smallcaps}. Notably, *insulter* and *engueuler* are adversative, which may explain their preference for [se faire]{.smallcaps}. Conversely, the most frequent lemmas with [être]{.smallcaps} are *appeler*, *inscrire*, *rappeler*, and *représenter* (each with 4 occurences). -\> **adversativity**?
...
ggplot(passive_sample_clean, 
  ...
  geom_bar(position = "dodge") +  
  facet_wrap(~ passive_variant) +  
  labs(title = "Verb Cluster and Adversativity in the Passive Variants",
       x = "Adversativity",
       y = "Number of Observations", 
       fill = "Verb Cluster") +
...
[être]{.smallcaps} is predominantly used with neutral, non-adversative verbs.
