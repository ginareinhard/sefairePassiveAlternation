...
Following @bohmann_large-scale_2023 [35] who claim that "only cases where both variants can be used interchangeably should be considered", we aim to exclude constructions that do not allow for a paraphrase with the other. Howeve
...
We have extracted the concordances from the French ParlaMint corpus [@11356/1864] using a CQL expression SketchEngine. ParlaMint consists of French parliamentary debates from 2017 to 2020 and contains 38,702,395 tokens (32,214,147 words). We chose this corpus because it is well-documented, POS-tagged and represents authentic French spoken language. We recognize that the genre of parliamentary debates is quite specialised and therefore results therefore cannot be generalized to all French spoken language. However, the language of parliamentary debates a fascinating genre in itself and is expected to reflect a wide range of topics, thus providing a cross-section of language varieties as they occur in French speech.

To extract the data we chose a semi-automatic approach that included manual filtering.
