...
In its standard form, the se faire passive construction consists of the pronoun "se", the conjugated form of the verb "faire" and a verb in the infinitive. According to @labelle_anticausativizing_2013 [235] the construction is "formally causative, but semantically passive". Similar to the être passive, the subject of the passive sentence is the experiencer of the action. As with être, the direct object of the active sentence becomes the subject of the passive sentence. *Language example: active sentence with two (animate) arguments, se faire passive sentence, evtl. etre passive sentence*
...
# agent pp column wenn passive variant se faire ist -> nimm den Wert aus column "agent pp", wenn passive variant etre ist gehe so vor wie bei den anderen, also wenn annotator A ist, nimm den Wert aus agent_pp_a, wenn Annotator  B ist nimm den Wert aus agent_pp_b
...
ordering levels: by default alphabetical; reference level always the first, i.e.:

-   passive variant: être

-   subj_animacy: animate, inanimate (better: inanimate?)
...
tab_model(passive_glmer) # Marginal R2 / Conditional R2	0.598 / 0.746
```
...
summary(model_1) #AIC: 790.7(higher i.e. slightly worse?)
tab_model(model_1) # Marginal R2 / Conditional R2	0.599 / 0.747 (minimally higher, i.e. slightly better?)
```
...
summary(model_2) #AIC: 786.3 (lower than model 1, minimally higher than max model)
tab_model(model_2) # Marginal R2 / Conditional R2	0.596 / 0.746 (minimally lower than tow previous model (but almost identical with fewer factors!))
...
# C 0.9549173 - pretty good I think?
```
