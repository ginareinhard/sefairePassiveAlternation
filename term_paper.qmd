...
Looking for an explanation within our theoretocal framework, we examined the most frequent communication verbs with [se faire]{.smallcaps} in our sample. The two most frequent lemmas (*insulter* and *engueuler*) carry a dynamic and adversative nuance and typically occur with an animate undergoer. These verbs may therefore behave similarly to hitting-touching verbs, emphasising the subject’s affectedness. This is supported by the finding that [se faire]{.smallcaps} is more frequent with hitting-touching verbs than [être]{.smallcaps}, but this effect is also not statistically significant. Alternatively, other factors could explain these findings. Factors to consider could be e.g. the register and medium: previous research has shown that [se faire]{.smallcaps} is more common in informal, spoken registers, whereas [être]{.smallcaps} is more frequently used in formal, written contexts (**sources**). Verbs like *insulter* and *engueuler* could be characteristic of colloquial speech.

An alternative explanation for this result is the inherent lexical properties of the verbs. In our model, the random effect of the lemma accounts for a considerable part of the variance (@sec-X), suggesting that certain verbs are more likely to occur with [se faire]{.smallcaps} due to their lexical properties. This aligns with Bohmann et al.’s findings [-@bohmann_large-scale_2023: 48] that English **get** is largely lexically determined. It emphasises the important role of lexical effects in the occurrence of [se faire]{.smallcaps}. We believe that the application of memory-based learning methods, such as k-nearest neighbour classifiers, could provide deeper insights into the lexical conditioning of [se faire]{.smallcaps}, as they capture lexical properties without requiring explicit rule-based abstractions (**source?**).
