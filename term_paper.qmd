...
The levels of each factors are ordered alphabetically by default. We rearranged the reference levels to be most useful for both modelling and interpreting: Where possible, we chose the most standard or unmarked case as the reference level (passive variant: être; complément d' agent: 0; modal: no; TAM: non-progressive, cluster: neutral). If this would negatively affect the stability of the model, we instead selected the most frequent level (subject animacy: animate, subject gender: masculine). In the case of adversativity, the most balanced level (adversative) was chosen to facilitate interpretation and maintain consistency with other factors.
...
: Overview of variables for the [être]{.smallcaps}: [se faire]{.smallcaps} passive alternation
...
The QQ-Plot reveals a significant deviation ($p$ = 0.02965) as tested by the Kolmogorov-Smirnov test, which may point to unreliable inferential measures. However, $p$-values depend (among other factors) on sample size and the KS test is reportedly highly sensitive to minor deviations **(reference),** many of which naturally occur in large datasets such as ours. Based on the overall distribution of residuals, we assess the magnitude of this deviation to be minor. <!-- https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html -->
...
We only have categorical data; therefore mean and standard deviation are not meaningful.
