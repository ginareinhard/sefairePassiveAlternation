...
sefaire_sample_annotated <- sefaire_sample_annotated |> 
  mutate(Reference = NULL, Kommentar = NULL) |> 
...
# remove now redundant "Reference" column, remove now redundant "Reference", remove "Kommentar" column, put new "reference" column in first position
etre_sample_annotated <- etre_sample_annotated |> 
  mutate(Reference = NULL, Kommentar = NULL) |> 
  select(reference, speaker, everything())

#still some differences left: être has an "ID" column which se faire does not; être has two agent_pp columns because it was annotated by A and B while se faire only has one, since agent_pp was annotated there by one person
```
...
check for typos/ inconsistencies in tagging in combined data
...
# summary
summary(passive_sample_combined)
#looks good, two "animate?" in subj_animacy_A
```

add "NA" as value for all cases that are only annotated by one person

```{r add-NA}
passive_sample_combined[passive_sample_combined == ""] <- NA
```
...
mean_kappa #0.8403021, very high agreement :)
