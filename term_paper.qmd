...
-   se faire favoured by dynamic verbs (potentially especially by verbs describing violent actions),

-   favoured by actions that are in process (progressive)

<!-- establish research problem -> show research gap and how our study aims to fill that (method)-->

As demonstrated, there exists extensive research on the alternation and the subtle differences of both participating passive variants. However, to our knowledge, no study thus far has investigated the exact factors predicting [se faire]{.smallcaps} over [être]{.smallcaps} using logistic regression modelling. Our study aims to fill that research gap: Applying a quantitative, multivariate, probabilistic approach to model corpus data, we investigate the factors that influence the choice between se faire and être in the passive alternation in spoken French, mpre precisely in parliament debates. Specifically, we aim to understand what factors contribute to speakers’ preference for one construction over the other. Regression modeling is useful in linguistic analysis because it captures the interrelatedness of various factors, and logistic regression modelling is particularly well-suited for linguistic analysis of alternations [@gries_statistics_2021: 238; @levshina_how_2015: 253-254]. It has been successfully employed for investigation of passive variation in other languages, such as English [@bohmann_large-scale_2023; @hundt_be-_2024], Swedish [@cuypere_corpus-based_2014] and Chinese [@guan_multifactorial_2024]. Therefore, such a model promises to yield insightful results which may contribute to the discussion of the passive alternation, potentially revealing new insights into the use of both [se faire]{.smallcaps} and [être]{.smallcaps}.
