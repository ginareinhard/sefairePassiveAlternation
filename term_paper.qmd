...
We expect phrases with *de* to have an even stronger effect than phrases with *par*, as they typically refer to less agentive or abstract sources (instead of agents), contradicting the dynamic nature of the *se faire* passive, which typically occurs with adversative or strongly agentive actions. To examine this, we will test the interaction between the compément d'agent and the verb cluster and adversativity??, as we expect the *se faire* passive to be favoured by dynamic and adversative verbs (see seq-XXX).
