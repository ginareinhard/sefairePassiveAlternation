...
We fit a mixed-effects binomial logistic regression model in R following Levshina [-@levshina_how_2015: 253-276] and Gries [-@gries_statistics_2021: 393-452]. The model calculates the log of the odds that a given construction is realized with „se faire“. To control for individual variation, we include the verb lemma and the speaker (as indicated in the meta data) as random effects. We set a significance threshold p-value of 0.05.

::: panel-tabset
#### Full Model

First, we build maximal model that includes all the factors, interactions and random effects described in sec.
...
The AIC is 732.5, the R^2^ is 0.460 without considering random effects (marginal) and 0.670 with random effects.

```{r full-passive-glmer-tab, echo = FALSE}
tab_model(full_passive_glmer)
```

#### Model Back-1

In the first step, all non-significant interactions ($cluster:adversativity$, $subjAnimacy:cluster$, $subjAnimacy:subjGender$) are removed. With this step, the AIC improves to 725.4 with an R^2^ of 0.450 / 0.664. Thus, without the interactions, the model accuracy increased while the variance that the model can explain decreased slightly. The categories `subj_gender`, `agent_pp`, `modal` and `tam` are non-significant. with p-values \> 0.05.
...
tab_model(model_back_1)
```

#### Final Model (Minimal model)

Non-significant fixed effects were removed step-by step, however, only the final model is reported here. Despite subj_gender having a p-value of \>0.05, and therefore being non-significant, we decided to keep it in our model for theoretical reasons/ interpretation. The AIC of 719.2 shows that removing `agent_pp`, `modal` and `tam` enhanced the quality of the final model while the R^2^ stayed constant at 0.451 / 0.662.
...
tab_model(final_model) #Marginal R2 / Conditional R2	0.451 / 0.662
```
:::
...
Looking at the log odds, it becomes apparent that
...
A positive coefficient means that the level boosts the chance of se faire (and therefore, decreases the chance of être).
...
The intercept of the final model is 2.78. This means that if the subject is animate and masculine, the cluster of the verb is neutral and the construction has an adversative sense, the chances of se faire are 2.78 times greater than être.

R-squared not recommended as central measure @levshina_how_2015 [259], Hosmer & Lemeshow (2000: 167)

significant effects in final_model_without_unknown (final model?):

-   cluster\[communication\];

-   subj animacy \[inanimate\] × adversativity \[adversative\]
...
# C 0.9414132 - pretty good I think? "outstanding discrimination" according to Hosmer & Lemeshow (2000: 162) in Levshina (2015: 259)
```

overall significance test for the model by comparing 0 model to full model and use an anova between those @gries_statistics_2021 [445-446]
