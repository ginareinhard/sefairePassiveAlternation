...
One general limitation of our dataset is that the predictions of our model do not fully reflect actual language use, as [être]{.smallcaps} is far more frequent than [se faire]{.smallcaps}. In the ParlaMint 2.1 corpus, [être]{.smallcaps} occurs 324,836 times (**38.300 in ?**), compared to just 1,254 instances of [se faire]{.smallcaps}. To prevent the model from being skewed, we created a balanced dataset based on the lemmas occuring with [se faire]{.smallcaps}[^4]. While this approach allowed us to focus on the factors influencing the passive alternation, it introduced an artificial ratio that does not represent real-life language use, where [être]{.smallcaps} is the unmarked default passive and [se faire]{.smallcaps} is a marked and rare variant. In addition, selecting lemmas based on their occurrence with [se faire]{.smallcaps} may have biased the lexical distribution towards verbs that are particularly compatible with [se faire]{.smallcaps}, thereby limiting the generalisability of our findings. We also excluded all occurrences with "unknown" gender to avoid potential bias and collinearity in the model. However, this led to further imbalance in the number of [être]{.smallcaps} and [se faire]{.smallcaps} constructions in the final dataset, which may have additionally limited the generalisability of our results.
...
Despite these limitations, the model diagnostics indicate a good overall fit, although certain aspects, such as suboptimal fits for specific data subsets (as shown by the binned residuals), suggest potential areas for refinement. The bootstrap validation supports our model's stability, but indicates that the results should be interpreted with caution. We chose to retain the original model for interpretation, recognising the trade-off between stability and fit, while noting that the simplified model (@tbl-model-without-speaker), which omits the speaker effect, may be a more robust alternative. Most fixed effects show minimal bias, although the bias for subject animacy (animate) may suggest that its effect may be slightly less reliable than the others. The verb cluster predictors have higher standard errors, which indicates some uncertainty. Overall, the estimates appear meaningful, but further work would be required to refine the model.

While bootstrap validation provides insight into model stability, applying additional validation techniques would contribute to identifying overfitting and ensuring that the model performs well on unseen data. One possible approach would be cross-validation, where the dataset is split into several subsets, the model is trained on each subset and tested on the remaining data.

To summarise, although the French [être:se faire]{.smallcaps} alternation presents considerable analytical challenges due to its complexity, our findings offer empirical evidence for the influence of subject animacy, adversativity, and aspectuality on this alternation. The results show that [se faire]{.smallcaps} is not exclusively used with animate subjects and suggest that it may emphasise the subject's responsibility or affectedness more strongly than [être]{.smallcaps}. The limitations of our dataset, annotation scheme, and model design, particularly with regard to gender, aspect and lexical variation, require further refinement. Methodologically, our model shows reasonable robustness, but additional validation techniques such as cross-validation would help to better assess generalisability and possible overfitting. Overall, this study offers a basis for a more refined analysis of the [être:se faire]{.smallcaps} passive alternation in French.
