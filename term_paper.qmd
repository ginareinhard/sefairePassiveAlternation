...
The `DHARMa` dispersion plot @fig-dispersion illustrates that the residuals are approximately normally distributed. The results of the dispersion test indicates no evidence of over- or underdispersion (dispersion = 0.97, $p$ = 0.632). Since the p-value is greater than 0.05, the "null hypothesis of normality cannot be rejected" [@levshina_how_2015: 162] suggesting that deviations are non significant.
...
The logistic regression outputted log odds, which were automatically converted to odds ratios by exponation in the `tab( )` function. However, we examined the log odds to identify tendencies of passive preference. Looking at the log odds of the significant variables, it becomes apparent that communication verbs boost the chance of [se faire]{.smallcaps} being used, while [être]{.smallcaps} is more likely in non-adversative contexts with inanimate subjects. (Although non-significant, feminine subjects are more likely to occur with [se faire]{.smallcaps} than masculine subjects.)
...
Since odds ratios are easier to interpret than log odds, we only used odds ratios when analysing the effects in detail.
...
kbl(freq_table, col.names = c("Factor", "Level", "être (absolute)", "se faire (absolute)", "être (relative)", "se faire (relative)"))
