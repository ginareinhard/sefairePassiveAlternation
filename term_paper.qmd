...
The French construction *se faire* followed by an infinitive has evolved through analogy with the passive voice. Some Originally, it had a causative meaning, where the subject played an active role in the action. However, in some contexts, it can also be interpreted as passive, alternating with the *être* passive construction.
...
Sometimes both readings are possible and the ambiguity cannot be resolved [@koenig_semantic_1992: 144], especially with cognition and perception verbs [@bellec_construction_2014: 206-207]. Therefore, despite its productivity as a passive construction, Raineri [-@raineri_analyse_2010: 356-357] concludes that [se faire]{.smallcaps} is problematic because it does not fit into two distinct categories nor does it function as a single weak causative. She points to the possibility of multiple readings [@kokutani_sur_2005] or of a continuum between causative and passive [-@raineri_analyse_2010: 89, 154-166].
...
: Overview of variables for the [être]{.smallcaps}: [se faire]{.smallcaps} alternation {.striped .hover .responsive #tbl-levels}
...
@fig-dharma-dispersion illustrates that the residuals are slightly right-skewed but approximately normally distributed. The results of the dispersion test indicate no evidence of over- or underdispersion (dispersion = 0.97, $p$ = 0.632). Since the $p$-value is greater than 0.05, the "null hypothesis of normality cannot be rejected" [@levshina_how_2015: 162], suggesting that deviations are non-significant.

```{r residuals-diagnostics-DHARMa-qq, echo = FALSE}
#| label: fig-dharma-qq
#| fig-cap: QQ-Plot to test deviation of residuals
plotQQunif(sim_res)
```

The QQ-Plot (@fig-dharma-qq) reveals a significant deviation ($p$ = 0.02965) as tested by the Kolmogorov-Smirnov test, which may point to unreliable inferential measures. However, $p$-values depend (among other factors) on sample size and the KS test is reportedly highly sensitive to minor deviations **(reference),** many of which naturally occur in large datasets such as ours. Based on the overall distribution of residuals, we assess the magnitude of this deviation to be minor. The outlier test is negative, showing no significant deviations from the expected distribution.
...
freq_animacy <- passive_sample_clean |>
  ...
  ungroup() |> 
  dplyr::select(subj_animacy, n_être, relative_être, `n_se faire`, `relative_se faire`)
...
neutral_etre_rel <- freq_cluster |>
  filter(cluster == "neutral") |> 
  pull(relative_être)

neutral_sefaire_rel <- freq_cluster |>
  filter(cluster == "neutral") |> 
  pull(`relative_se faire`)

communication_etre_rel <- freq_cluster |>
  filter(cluster == "communication") |> 
  pull(relative_être)

communication_sefaire_rel <- freq_cluster |>
  filter(cluster == "communication") |> 
  pull(`relative_se faire`)

hittingtouching_etre_rel <- freq_cluster |>
  filter(cluster == "hitting-touching") |> 
  pull(relative_être)

hittingtouching_sefaire_rel <- freq_cluster |>
  filter(cluster == "hitting-touching") |> 
  pull(`relative_se faire`)
...
freq_animacy_bind <- freq_animacy
colnames(freq_animacy_bind)[1] <- "level"
freq_animacy_bind$factor <- "Animacy"

freq_gender_bind <- freq_gender
colnames(freq_gender_bind)[1] <- "level"
freq_gender_bind$factor <- "Gender"

freq_table <- bind_rows(freq_animacy_bind, freq_gender_bind) |> 
...
kbl(freq_table, col.names = c("Factor", "Level", "absolute", "relative", "absolute", "relative")) |> 
  ...
  # row_spec(1:2, background = "#F2F0EF") |> 
  # row_spec(3:4, background = "white")
...
Logistic regression produced log odds, which were automatically converted to odds ratios by exponentiation during model selection using the `tab()` function. Nevertheless, we examined the log odds to identify tendencies of passive preference. 

```{r coefficients-log-odds, echo = FALSE}
final_model_coef <- coefficients(summary(final_model))[,"Estimate"]
final_model_coef_table <- data.frame(
  Factor = names(final_model_coef),
  Estimate = round(unname(final_model_coef), 3)
)
```

```{r coefficients-log-odds-tab, echo = FALSE}
#| label: tbl-log-odds
kbl(final_model_coef_table, caption = "Final model log odds")
```

Looking at the log odds of the significant variables in @tbl-log-odds, it becomes apparent that communication verbs increase the likelihood of [se faire]{.smallcaps} being used over [être]{.smallcaps]. [être]{.smallcaps} is more likely than [se faire]{.smallcaps} in non-adversative contexts with inanimate subjects. Feminine subjects favour [se faire]{.smallcaps} over [être]{.smallcaps], but the effect was not significant.
...
Since odds ratios are easier to interpret than log odds, we only used odds ratios when analysing the effects in detail. As can be seen in @tbl-final-model, the intercept of the final model is `r round(final_model_odds_ratios[1], 2)`. This means that in the context of the reference levels (subject is animate and masculine, the cluster of the verb is neutral and the construction has an adversative sense), the chances of [se faire]{.smallcaps} being chosen are `r round(final_model_odds_ratios[1], 2)` times greater than [être]{.smallcaps}. <!-- The exact values of odds ratios are interpreted with regard to our hypotheses in @sec-results. -->
