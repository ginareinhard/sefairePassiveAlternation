...
Looking at the most frequent communication verb lemmas in our data in @tbl-cluster-lemmas, we find that *insulter* (10 occurrences), *engueuler* (7 occurrences), and *représenter* (5 occurrences) appear most often with [se faire]{.smallcaps}. Notably, *insulter* and *engueuler* are adversative and generally occur with an animate undergoer, which may explain their preference for [se faire]{.smallcaps}. Conversely, the most frequent lemmas with [être]{.smallcaps} are *appeler*, *inscrire*, *rappeler*, and *représenter* (each with 4 occurences), which are annotated as non-adversative in our dataset, except for one occurence.
...
Similarly to the adversativity of the verb, the animacy of the subject could account for communication verbs like *insulter* and *engueuler*, which typically occur with an animate undergoer. @fig-interaction-anim-cluster shows the absolute Frequencies of [être]{.smallcaps} and [se faire]{.smallcaps} across verb clusters and subject animacy.
