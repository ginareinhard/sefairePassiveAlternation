...
To **extract** our sample we chose a semi-automatic approach that included extraction via CQL expression as well as manual filtering. Our aim was to select an equal number of occurrences of [être]{.smallcaps} and [se faire]{.smallcaps} constructions.

For this, we first automatically extracted a sample of 1,254 [se faire]{.smallcaps} constructions from the French ParlaMint corpus [@11356/1431] using a CQL expression. After this automatic extraction, we manually filtered the data to exclude false-positive constructions, i.e. constructions that did not fulfil our predefined criteria for [se faire]{.smallcaps} passives (see @sec-filtering-criteria). After manual filtering, the sample comprised 535 [se faire]{.smallcaps} constructions. We annotated the lemmas of these constructions and made a list of them.
...
We also extracted the [être]{.smallcaps} constructions by using a CQL expression in SketchEngine. We then filtered the concordances using the list of lemmas we had previously collected from the [se faire]{.smallcaps} sample. Due to the download limit of SketchEngine, we downloaded these constructions in four subsamples of 9295-9794 concordances. All filtered instances stored in a .csv file and merged to one dataframe in R. To ensure reproducibility, we set a seed using the function `setseed( )` to randomly sample the data. The seed was determined by the lottery numbers on Wednesday, 12^th^ March 2025 as provided on the Website <https://www.lotto.de/lotto-6aus49/lottozahlen> (removing one digit because they were too long). We then took a random sample from the concordance lines that was equal in size to the manually filtered amount of [se faire]{.smallcaps} constructions (i.e. **535 -\> 532 (**e**xplain reduction**)). The constructions were then manually annotated for the features listed in @sec-factors, in order to prepare the data for statistical modelling.
...
### Manual filtering exclusion criteria {#sec-filtering-criteria}
